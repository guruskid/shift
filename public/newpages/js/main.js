...
const countries_iso = [
  ...
  for (let i = 0; i < countries_iso.length; i++) {
    let option = `<option value=${countries_iso[i].name}>${countries_iso[i].flag} ${countries_iso[i].name.trim()}</option>`
...
$("#addcard_button").on("click", function() {
    ...
    let price_per_card = $("#price_per_card")
        .text()
        .trim();
    let quantity = $("#qty_input").val().trim();
...
// countryISOToFlagEmoji('NG')
// countryISOToFlagEmoji('US')