                        ...
                        <div class="page-title-heading">
                            <div class="page-title-icon" style=" height: 150px; width: 150px ">
                                <a href="{{ asset('storage/avatar/' . $user->dp) }}"><img
                                        src=" {{ asset('storage/avatar/' . $user->dp) }} " height="120px"
                                        alt=""></a>
                            </div>
                            ...
                            <div class="card card-body">
                                ...
                                @if ($user->nairaWallet)
                                    <div class="d-flex justify-content-between">
                                        <h6> ₦{{ number_format($user->nairaWallet->amount) }} </h6>
                                        <h6>Debt: ₦{{ number_format($user->debt) }}</h6>
                                    </div>
                                @else
                                    No Naira wallet
                                @endif
                        ...
                        <div class="main-card mb-3 card">
                            <div class="card-body">
                                <ul class="nav nav-tabs nav-justified">
                                    ...
                                    <li class="nav-item"><a data-toggle="tab" href="#debt-tab" class="nav-link">Debt</a>
                                    </li>
                                </ul>
                                <div class="tab-content">
                                            ...
                                            <table class="mb-2  table " id="nt-table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <strong>₦{{ number_format($user->nairaWallet->amount) }}
                                                            </strong>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <strong>₦{{ number_format($ledger->balance) }} </strong>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-center"><strong>Debit Total</strong></td>
                                                        <td class="text-center"><strong>₦{{ number_format($ledger->dr) }}
                                                            </strong></td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-center"><strong>Credit Total</strong></td>
                                                        <td class="text-center"><strong>₦{{ number_format($ledger->cr) }}
                                                            </strong></td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-center"><strong>L-Log</strong></td>
                                                        <td class="text-center"><strong> {{ $log->count() }} -
                                                                ₦{{ number_format($log->sum('amount')) }} </strong></td>
                                                    </tr>
                                                    <tr>
                                                ...
                                                <tbody>
                                                    @foreach ($wallet_txns as $t)
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                ₦{{ number_format($t->previous_balance) }}</td>
                                                            <td class="text-center">
                                                                ₦{{ number_format($t->current_balance) }}</td>
                                            ...
                                            @foreach ($verifications as $v)
                                                <div class="col-md-4">
                                                    <a href="{{ asset('storage/idcards/' . $v->path) }}">
                                                        <img src="{{ asset('storage/idcards/' . $v->path) }}"
                                                            class="img-fluid">

                                                    </a>
                                                    <h6>{{ $v->type }} <span
                                                            class="badge badge-primary">{{ $v->status }}</span>
                                                    </h6>
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    {{-- Debt --}}
                                    <div class="tab-pane" id="debt-tab" role="tabpanel">
                                        <form action="{{ route('admin.add-debt', $user) }}" method="POST"> @csrf
                                            <div class="row ">
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <label for="">Account payment was made to *</label>
                                                    <select name="account_id" required class="form-control">
                                                        <option value="">Select account </option>
                                                        @foreach ($user->accounts as $acct)
                                                            <option value="{{ $acct->id }}">{{ $acct->bank_name }}
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <label for="">Reason for recovery *</label>
                                                    <select name="reason" id="" class="form-control">
                                                        <option value="">Select reason</option>
                                                        <option>Excess payment</option>
                                                        <option>Inconsistent wallet balance</option>
                                                        <option>Unauthorized credit transaction</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-4 mb-3">
                                                    <label for="">Number of times payment was made *</label>
                                                    <input type="number" name="frequency" class="form-control">
                                                </div>
                                                <div class="col-md-4 mb-3">
                                                    <label for="">Date / Time (Leave blank to use current date
                                                        time)</label>
                                            ...
                                            <button class="btn btn-primary">Save debt to user account</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
