<template>
    <div class="modal fade " id="new-tron-wallet">
                ...
                <form @submit.prevent="createWallet">
                    <div class="modal-body p-4">
                        ...
                        <button :disabled="loading" class="btn btn-block c-rounded bg-custom-gradient">
                            {{ loading ? 'Processing...' : 'Create Tron Wallet' }} </button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            createWallet() {
                this.loading = true;
                axios.post('/user/tron/create', {'pin' : this.pin } )

                .then((res)=>{
                    if (res.data.success) {
                        swal('Great!!', 'Tron walet created successfully', 'success');
